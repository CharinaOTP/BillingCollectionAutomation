...
describe('JMS Form CSV Upload', () => {
  ...
  cy.get('[data-row-key="02-200001-7"] > :nth-child(3)', { timeout: 10000 })
  .should('be.visible')
  .dblclick();
      cy.wait(4000)
       ...
       cy.contains('JMS Transactions')
        ...
        .click()
          ...
          // READ CSV HERE
            cy.fixture('jmsadd.csv').then((csvData) => {
              Papa.parse(csvData, {
              header: true,
              skipEmptyLines: true,
...
cy.get('.ant-select-dropdown:not(.ant-select-dropdown-hidden)')
  ...
  .click();
    ...
    cy.wait(4000)
    // Read CSV
    cy.fixture('jmsmaintenance.csv').then((csvData) => {
...
cy.get(':nth-child(4) > .ant-row > .ant-col > .ant-form-item-control-input > .ant-form-item-control-input-content > .ant-picker')
  ...
  .type(`${feedbackDate}{enter}`, { delay: 300 });
        ...
        cy.get('[data-row-key="185"] > :nth-child(2)')
           .should('be.visible')
        .click()
          cy.wait(4000)
